...
import { regionService, RegionInfo, TOP_REGIONS } from '../services/regionService';
...
export const LeaderboardScreen: React.FC<LeaderboardScreenProps> = ({
  ...
  // Subscribe to regionService changes & trigger initial GPS scan
  useEffect(() => {
    ...
    // Detect GPS on mount
    regionService.requestGpsDetection().then(() => {
      const coords = regionService.getCachedCoords();
      if (coords) {
        setCoordsString(`${coords.lat.toFixed(2)}°, ${coords.lon.toFixed(2)}°`);
      }
  ...
  const handleManualRegionSelect = (code: string) => {
  ...
  const handleDetectGps = async () => {
    ...
    await regionService.requestGpsDetection();
    ...
    <div className="w-full max-w-md mx-auto px-4 pt-2 pb-24 text-slate-100 flex flex-col gap-3 select-none">
      ...
      <header className="px-1 pt-1 pb-1 flex flex-col gap-2.5">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-black tracking-tight text-white flex items-center gap-2">
              Leaderboard
              <span className="inline-block w-2 h-2 rounded-full bg-sky-400 shadow-[0_0_8px_#38bdf8]" />
            </h1>
          ...
          <div className="neu-inset px-2.5 py-1.5 rounded-full flex items-center gap-1.5 border border-white/[0.04]">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-[11px] font-medium text-slate-300">Live Sync</span>
          </div>
        </div>

        {/* Region & Network GPS Detection Bar (Replaces old scope and timeframe filters) */}
        <div className="neu-card rounded-2xl p-3 border border-white/[0.05] flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-bold uppercase tracking-wider bg-sky-500/15 text-sky-300 border border-sky-500/30 px-1.5 py-0.5 rounded-md flex items-center gap-1">
          ...
          {/* Change / Inspect Top 10 Regions Button */}
          <button
            onClick={() => {
              if (soundEnabled) soundService.playClick();
              setShowRegionPicker(true);
            }}
            className="neu-inset px-2.5 py-1.5 rounded-xl text-[11px] font-bold text-sky-400 hover:text-sky-300 active:scale-95 transition-all flex items-center gap-1 shrink-0 cursor-pointer border border-sky-500/20 shadow-inner"
          >
        ...
        <div className="flex items-center justify-between px-0.5">
          <div className="neu-flat px-3 py-1 rounded-xl flex items-center gap-1.5 border border-white/[0.03]">
          ...
          <span className="text-[10px] font-semibold bg-sky-950/80 text-sky-300 px-2.5 py-1 rounded-xl border border-sky-500/20">
          ...
          {(['all', 'easy', 'medium', 'hard'] as DifficultyFilter[]).map((diffKey) => {
            ...
            return (
              <button
                ...
                className={`flex-1 py-1.5 rounded-xl text-[11px] font-bold tracking-wider transition-all text-center ${
                  isActive
                    ? 'neu-active-glow text-white shadow-sm'
        ...
        <main className="flex-1 space-y-4 pt-1">
          ...
          {podium.length > 0 && (
            ...
            >
              <div className="flex items-center justify-between pb-3 mb-2 border-b border-white/[0.05]">
                ...
                <span className="text-[10px] font-semibold text-emerald-400 bg-emerald-950/80 px-2 py-0.5 rounded-md border border-emerald-500/20">
                  Top 0.1% Focus
                </span>
              </div>
              ...
              <div className="flex items-end justify-between gap-2 pt-2">
                ...
                {podium[1] ? (
                  <div className="flex-1 flex flex-col items-center">
                    <div className="mb-1 flex flex-col items-center">
                      <span className="text-[9px] font-extrabold uppercase px-1.5 py-0.5 rounded-md bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 flex items-center gap-0.5 mb-1">
                      ...
                      <div className="relative">
                        <div className="w-14 h-14 rounded-full p-[2px] bg-gradient-to-tr from-slate-400 via-slate-100 to-slate-500 shadow-[0_0_12px_rgba(203,213,225,0.4)]">
                          <div className="w-full h-full rounded-full bg-[#1c2127] flex items-center justify-center font-bold text-slate-200 text-base">
                            {podium[1].initials}
                          </div>
                        </div>
                    ...
                    <span className="text-xs font-bold text-sky-400 mt-2 flex items-center gap-0.5 truncate max-w-[85px]">
                      {podium[1].isUser ? 'You' : podium[1].name}
                      {podium[1].isUser && <Crown className="w-3 h-3 text-amber-400" />}
                    </span>
                    <span className="text-sm font-black text-white mt-0.5 tracking-tight">
                ...
                {podium[0] && (
                  <div className="flex-1 flex flex-col items-center -translate-y-2">
                    <div className="mb-1 flex flex-col items-center">
                      <Trophy className="w-5 h-5 text-amber-400 drop-shadow-[0_0_8px_#facc15] mb-0.5 animate-bounce" />
                      ...
                      <div className="relative">
                        <div className="w-16 h-16 rounded-full p-[2.5px] bg-gradient-to-tr from-amber-500 via-yellow-200 to-amber-600 shadow-[0_0_18px_rgba(250,204,21,0.5)]">
                          <div className="w-full h-full rounded-full bg-[#201d12] flex items-center justify-center font-extrabold text-amber-300 text-lg">
                            {podium[0].initials}
                          </div>
                        </div>
                    ...
                    <span className="text-xs font-bold text-white mt-2 truncate max-w-[95px]">
                      {podium[0].isUser ? 'You' : podium[0].name}
                    </span>
                    <span className="text-base font-black text-amber-300 mt-0.5 tracking-tight">
                    ...
                    <div className="w-full h-24 bg-gradient-to-b from-[#21262d] to-[#161a1d] rounded-t-xl mt-3 flex flex-col items-center justify-center border-t-2 border-amber-400/50 shadow-inner">
                ...
                {podium[2] ? (
                  <div className="flex-1 flex flex-col items-center">
                    <div className="mb-1 flex flex-col items-center">
                      <span className="text-[9px] font-bold text-orange-400 bg-orange-950/40 px-1.5 py-0.5 rounded border border-orange-500/20 mb-1">
                      ...
                      <div className="relative">
                        <div className="w-14 h-14 rounded-full p-[2px] bg-gradient-to-tr from-amber-700 via-orange-300 to-yellow-800 shadow-[0_0_12px_rgba(249,115,22,0.3)]">
                          <div className="w-full h-full rounded-full bg-[#201815] flex items-center justify-center font-bold text-orange-200 text-base">
                            {podium[2].initials}
                          </div>
                        </div>
                    ...
                    <span className="text-xs font-bold text-slate-200 mt-2 truncate max-w-[85px]">
                    ...
                    <div className="w-full h-12 neu-inset rounded-t-xl mt-3 flex items-center justify-center border-t border-orange-500/30">
                      <span className="text-[10px] font-bold text-slate-400">3rd</span>
                    </div>
                  </div>
                ) : (
                  <div className="flex-1" />
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-semibold text-emerald-400 bg-emerald-950 px-1.5 py-0.2 rounded border border-emerald-500/30">
                    +0.6%
                  </span>
                </div>
                ...
                <div className="mt-2.5 pt-2 border-t border-white/[0.04] flex items-center justify-between text-[11px]">
                  ...
                  <button
                    onClick={onPlayNow}
                    className="text-xs font-semibold text-sky-400 hover:text-sky-300 transition-colors flex items-center gap-0.5 cursor-pointer"
                  >
                    <span>Play Now</span>
          ...
          {contenders.length > 0 && (
            <section aria-label="Regional Contenders" className="space-y-2.5">
              <div className="flex justify-between items-center px-1">
                ...
                <span className="text-[11px] text-slate-400">Steadiness Score</span>
              </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-slate-300 to-slate-500 flex items-center justify-center text-slate-950 font-extrabold text-sm shadow-inner shrink-0">
                        {rankNumber}
                      </div>
          ...
          <section className="pt-2 pb-1 flex justify-center">
            <button
              ...
              className="w-full py-3.5 px-5 rounded-2xl neu-flat hover:bg-[#1f2429] active:scale-[0.98] transition-all flex items-center justify-center gap-2.5 border border-sky-500/20 group shadow-lg cursor-pointer"
            >
              <div className="w-7 h-7 rounded-lg bg-sky-500/10 flex items-center justify-center text-sky-400 group-hover:scale-110 transition-transform">
                {copiedInvite ? (
      ...
      {/* ========================================================================= */}
      {/* 3. Top 10 World Regions Modal                                             */}
      {/* ========================================================================= */}
      {showRegionPicker && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="neu-card w-full max-w-md rounded-t-3xl sm:rounded-3xl p-5 border border-white/10 max-h-[85vh] flex flex-col gap-4 animate-in slide-in-from-bottom duration-200">
            {/* Modal Header */}
            <div className="flex items-center justify-between pb-2 border-b border-white/[0.06]">
              <div className="flex items-center gap-2">
                <Globe2 className="w-5 h-5 text-sky-400" />
                <div>
                  <h3 className="text-base font-extrabold text-white">Top 10 Global Regions</h3>
                  <p className="text-[11px] text-slate-400">
                    Names &amp; leaderboards localized by region
                  </p>
                </div>
              </div>
              <button
                onClick={() => setShowRegionPicker(false)}
                className="w-8 h-8 rounded-full neu-inset flex items-center justify-center text-slate-400 hover:text-white"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <button
              ...
              className="w-full py-3 px-4 rounded-2xl bg-gradient-to-r from-sky-600 to-sky-500 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-md active:scale-98 transition-all"
            >
              <Navigation className={`w-4 h-4 ${isDetectingLocation ? 'animate-spin' : ''}`} />
              <span>
                {isDetectingLocation
                  ? 'Detecting Lat-Long from Network/GPS...'
                  : 'Auto-Detect My Region (GPS / Mobile Network)'}
              </span>
            </button>

            {/* Top 10 Regions List */}
            <div className="flex-1 overflow-y-auto space-y-2 pr-1 no-scrollbar max-h-[50vh]">
              {Object.values(TOP_REGIONS).map((reg) => {
                const isSelected = reg.code === currentRegion.code;
                return (
                  <button
                    key={reg.code}
                    onClick={() => handleManualRegionSelect(reg.code)}
                    className={`w-full p-3 rounded-2xl flex items-center justify-between text-left transition-all ${
                      isSelected
                        ? 'neu-active-glow border border-sky-400/40 bg-sky-950/40'
                        : 'neu-flat hover:bg-[#1f2429]'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          ...
                          {isSelected && (
                            <span className="text-[9px] font-extrabold text-sky-400 uppercase bg-sky-500/20 px-1.5 py-0.5 rounded">
                              ACTIVE
                            </span>
                          )}
                        </div>
                        <p className="text-[11px] text-slate-400">
                          {reg.players.slice(0, 3).map((p) => p.name).join(', ')}...
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="text-right">
                      <span className="text-xs font-bold text-slate-300">
                        {reg.activeWalkers.toLocaleString()}
                      </span>
                      <span className="block text-[9px] text-slate-500 uppercase">Walkers</span>
                    </div>
                  </button>
                );
              })}
